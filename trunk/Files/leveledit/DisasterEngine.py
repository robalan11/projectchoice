...
class DisasterEngine:
    ...
    textColor = (120, 120, 255)
    def __init__(self, screen, resolution):
        pygame.init()
        #load images
        
        Tile.standard_img = load_image("0F.png")
        ...
        self.background.fill(DisasterEngine.bgColor)
        ...
        #TowerLists
        for i in range(DisasterEngine.typesPerGrid):
    ...
    def resetGrid(self):
        ...
        DisasterEngine.grid=[]
        for i in xrange(DisasterEngine.gridHeight):
            DisasterEngine.grid.append([])
        for i in xrange(DisasterEngine.gridWidth):
            DisasterEngine.grid[0].append([".",".",".",".",".",".",".",".",".",".",".",".",".",".",".","."])
        for i in xrange(DisasterEngine.gridHeight):
            DisasterEngine.grid[i]=copy.deepcopy(DisasterEngine.grid[0])
    ...
    def render(self):
        #print "rendering"
        self.screen.fill(DisasterEngine.bgColor)
        ...
        alertLiteral = self.alertFont.render(str(self.alertText), 1, DisasterEngine.textColor)
    ...
    def sendAlert(self, alertText, alertLengthMillis):
        ...
        while pygame.time.get_ticks() < alerttime:
            alertLiteral = self.alertFont.render(str(self.alertText), 1, DisasterEngine.highlightColor)
    ...
    def highlightZone(self, zone):
        if self.mode=="grid":
            pygame.draw.rect(self.screen, DisasterEngine.highlightColor, pygame.Rect(zone[0]*32,zone[1]*32,32,32) )
    def convertMap(self, filename):
        print "Converting old formatted file:" , filename
        self.saveMap("Temp.txt")
        self.oldloadMap(filename)
        self.saveMap(filename)
        
        self.loadMap("Temp.txt")
    ...
    def oldloadMap(self, filename):
        ...
        for y in range(len(level)-1):
            for x in range(len(level[y])-1):
                for i in range(len(level[y][x])-1):
                    if(level[y][x][i]=='D' and i == 1):
                        towertype= [2, "D"]
                        self.createTile(y,x,towertype)
                        towertype= [1, "W"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='S' and i == 1):
                        towertype= [2, "S"]
                        self.createTile(y,x,towertype)
                        towertype= [1, "W"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='W' and i == 1):
                        towertype= [1, "W"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='D' and i == 2):
                        towertype= [4, "D"]
                        self.createTile(y,x,towertype)
                        towertype= [3, "W"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='S' and i == 2):
                        towertype= [4, "S"]
                        self.createTile(y,x,towertype)
                        towertype= [3, "W"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='W' and i == 2):
                        towertype= [3, "W"]
                        self.createTile(y,x,towertype)
                        
                        
                    elif(level[y][x][i]=='e' and i == 1):
                        towertype= [2, "D"]
                        self.createTile(y,x,towertype)
                        towertype= [1, "A"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='t' and i == 1):
                        towertype= [2, "S"]
                        self.createTile(y,x,towertype)
                        towertype= [1, "A"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='x' and i == 1):
                        towertype= [1, "A"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='e' and i == 2):
                        towertype= [4, "D"]
                        self.createTile(y,x,towertype)
                        towertype= [3, "A"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='t' and i == 2):
                        towertype= [4, "S"]
                        self.createTile(y,x,towertype)
                        towertype= [3, "A"]
                        self.createTile(y,x,towertype)
                    elif(level[y][x][i]=='x' and i == 2):
                        towertype= [3, "A"]
                        self.createTile(y,x,towertype)
                        
                        
                    elif(not level[y][x][i]=='.' and not level[y][x][i]=='\n'):
                        towertype= [i*2, level[y][x][i]]
                        self.createTile(y,x,towertype)
        ...
        while not self.gameover:
            ...
            for event in pygame.event.get():
                ...
                if event.type == KEYDOWN and event.key == K_l:
                    self.loadMap("ImportedMap.txt")
                if event.type == KEYDOWN and event.key == K_c:
                    self.convertMap("ConvertMap.txt")
